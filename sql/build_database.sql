...
DROP TABLE IF EXISTS journal_alternate_ids;
...
\copy all_article(journal, journal_title, journal_subtitle, article, issue, volume, epub_year, collection_year) from /data/all_articles.csv CSV DELIMITER '@' QUOTE '"' ENCODING 'utf-8';

UPDATE all_article SET journal = lower(trim(journal));
UPDATE all_article SET publication_year = left(collection_year, 4)::integer;
--UPDATE all_article SET issue = null where issue = 'n';
--UPDATE all_article SET volume = null where volume = 'v';
...
\copy journal_alternate_ids(download_id, referential_id) from /data/journal_alternate_ids.csv CSV DELIMITER ',' QUOTE '"' ENCODING 'utf-8';
